export const ANIME_DATA = [
    {
        id: '1',
        names: {
            original: 'One Piece',
            en: 'One Piece',
            fr: 'One Piece'
        },
        info: {
            year: '1999',
            episodes: '1100+',
            synopsis: {
                en: 'Monkey D. Luffy sets off on an adventure to find the legendary One Piece treasure.',
                fr: 'Luffy part à l\'aventure pour trouver le légendaire trésor One Piece.'
            }
        },
        images: [
            'https://picsum.photos/seed/op1/400',
            'https://picsum.photos/seed/op2/400',
            'https://picsum.photos/seed/op3/400',
            'https://picsum.photos/seed/op4/400',
        ],
        vignette: 'https://cdn.myanimelist.net/images/anime/1386/100624l.jpg',
    },
    {
        id: '2',
        names: {
            original: 'Naruto',
            en: 'Naruto',
            fr: 'Naruto'
        },
        info: {
            year: '2002',
            episodes: '220',
            synopsis: {
                en: 'Naruto Uzumaki, a young ninja who seeks recognition from his peers and dreams of becoming the Hokage.',
                fr: 'Naruto Uzumaki, un jeune ninja qui cherche la reconnaissance de ses pairs et rêve de devenir Hokage.'
            }
        },
        images: [
            'https://picsum.photos/seed/nar1/400',
            'https://picsum.photos/seed/nar2/400',
            'https://picsum.photos/seed/nar3/400',
            'https://picsum.photos/seed/nar4/400',
        ],
        vignette: 'https://cdn.myanimelist.net/images/anime/2/50745l.jpg',
    },
    {
        id: '3',
        names: {
            original: 'Shingeki no Kyojin',
            en: 'Attack on Titan',
            fr: "L'Attaque des Titans"
        },
        info: {
            year: '2013',
            episodes: '87',
            synopsis: {
                en: 'Humanity lives inside cities surrounded by enormous walls due to the Titans.',
                fr: 'L\'humanité vit dans des cités entourées de murs immenses à cause des Titans.'
            }
        },
        images: [
            'https://picsum.photos/seed/aot1/400',
            'https://picsum.photos/seed/aot2/400',
            'https://picsum.photos/seed/aot3/400',
            'https://picsum.photos/seed/aot4/400',
        ],
        vignette: 'https://cdn.myanimelist.net/images/anime/10/47347l.jpg',
    },
    {
        id: '4',
        names: {
            original: 'Boku no Hero Academia',
            en: 'My Hero Academia',
            fr: 'My Hero Academia'
        },
        info: {
            year: '2016',
            episodes: '138+',
            synopsis: {
                en: 'In a world where people are born with superpowers, a boy without them still dreams of becoming a hero.',
                fr: 'Dans un monde où les gens naissent avec des super-pouvoirs, un garçon sans pouvoirs rêve de devenir un héros.'
            }
        },
        images: [
            'https://picsum.photos/seed/mha1/400',
            'https://picsum.photos/seed/mha2/400',
            'https://picsum.photos/seed/mha3/400',
            'https://picsum.photos/seed/mha4/400',
        ],
        vignette: 'https://cdn.myanimelist.net/images/anime/10/78745l.jpg',
    },
    {
        id: '5',
        names: {
            original: 'Kimetsu no Yaiba',
            en: 'Demon Slayer',
            fr: 'Demon Slayer'
        },
        info: {
            year: '2019',
            episodes: '55+',
            synopsis: {
                en: 'Tanjirou Kamado sets out to become a demon slayer after his family is slaughtered.',
                fr: 'Tanjirou Kamado devient un pourfendeur de démons après le massacre de sa famille.'
            }
        },
        images: [
            'https://picsum.photos/seed/ds1/400',
            'https://picsum.photos/seed/ds2/400',
            'https://picsum.photos/seed/ds3/400',
            'https://picsum.photos/seed/ds4/400',
        ],
        vignette: 'https://cdn.myanimelist.net/images/anime/1286/99889l.jpg',
    },
];
